...
import { SET_FILES, SET_FILES_WITH_TYPES, SetFilesAction } from "../actions/files.actions";
...
export const setFilesEffect = (action$: any) => action$.pipe(
  ofType(SET_FILES),
  tap(() => console.log("Setting path type")), // debugging
  flatMap((action: SetFilesAction) => {
    return Promise.all(action.payload.map(async (el: string) => {
      return {
        name: el,
