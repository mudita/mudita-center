...
import { addDeviceWithGivenSerialNumber } from "E2E/helpers"
...
describe("E2E mock sample - overview view", () => {
  ...
  it("Connect 1st and 2ed device", async () => {
    addDeviceWithGivenSerialNumber("device-1")
    addDeviceWithGivenSerialNumber("device-2")

    await browser.pause(6000)

    // punkt 4 i 5

    // wybierasz urzadzenie
  ...
  it("Verify 1st device", async () => {
    const serialNumberElement = await OverviewPage.overviewSerialNumber
    await expect(serialNumberElement).toHaveText("device-1")
    //przepiac na 2ed device
  })

  it("Verify 2ed device", async () => {
    const serialNumberElement = await OverviewPage.overviewSerialNumber
    await expect(serialNumberElement).toHaveText("device-2")
  })

  it("Connect 3-6th device", async () => {
    addDeviceWithGivenSerialNumber("device-3")
    addDeviceWithGivenSerialNumber("device-4")
    addDeviceWithGivenSerialNumber("device-5")
    addDeviceWithGivenSerialNumber("device-6")
  })
