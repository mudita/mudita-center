...
import { isResponseSuccess, isResponseSuccessWithData } from "Core/core/helpers"
import {
  RequestResponse,
  RequestResponseStatus,
  SuccessRequestResponse,
} from "Core/core/types/request-response.interface"
import { Message, NewMessage, Thread } from "Core/messages/dto"
import {
...
export class MessageService {
  ...
  public async getMessage(id: string): Promise<RequestResponse<Message>> {
    const response =
      await this.deviceProtocol.device.request<GetMessageResponseBody>({
  ...
  public async getMessages(
    ...
    const response =
      await this.deviceProtocol.device.request<GetMessagesResponseBody>({
        body,
        endpoint: Endpoint.Messages,
        method: Method.Get,
      })
  ...
  private async createSingleMessage(
    ...
    const result =
      await this.deviceProtocol.device.request<CreateMessageResponseBody>({
