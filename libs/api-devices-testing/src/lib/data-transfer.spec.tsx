...
import { APIFileTransferService, APIDataTransferService, ServiceBridge } from "device/feature"
import { setActiveDevice } from "./helpers/protocol-validator"
import { DataTransferDomain } from "Libs/device/models/src"
import { getEmptyTransferData, getFullTransferData, getInvalidTransferData } from "./helpers/file-transfer-data"
...
describe("Data transfer", () => {
  ...
  function fetchSupportedFeatures() {
   ...
   async function performDataTransfer(base64Sources: { [key: string]: string } = {}, errorExpected: boolean) {
      const serviceBridge = new ServiceBridge()
      const filteredRestoreFeatures = importFeatures.filter(({ feature }) => (feature in base64Sources))
      ...
      const result = await apiDataTransferService.startPreDataTransfer({ domains: filteredRestoreFeatures.map(item => item.key)  })
      ...
      expect(Object.keys(domainsResponse).length).toEqual(filteredRestoreFeatures.length)
      ...
      const startRestoreResponse = await apiDataTransferService.startDataTransfer({ dataTransferId: dataTransferId })
      expect(startRestoreResponse.ok).toBeTruthy()
      let importResult =  true
      while (importProgress < 100) {
          const checkRestoreResponse = await apiDataTransferService.checkDataTransfer({ dataTransferId: dataTransferId })
