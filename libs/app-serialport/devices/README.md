...
1. Create a new class that extends the [`SerialPortDevice`](../main/src/lib/serial-port-device.ts) class.
...
4. Add a constructor accepting the object of [`SerialPortDeviceOptions`](../main/src/lib/serial-port-device.ts) type that calls the parent constructor using `super` method:
...
5. Optionally, set the `requestIdKey` property to instruct the [`SerialPortDevice`](../main/src/lib/serial-port-device.ts) class where to put a generated unique key used for identifying the requests.
