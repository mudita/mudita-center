...
export class AppSerialPortService {
  ...
  private detectChangesPromise?: Promise<void>
  ...
  private async initializeDevice(deviceInfo: SerialPortDeviceInfo) {
    ...
    freezeHandler.on("freeze", async () => {
      ...
      logger.log(`Device at id ${deviceInfo.id} is frozen. Pausing requests.`)
      if (device) {
    ...
    freezeHandler.on("unfreeze", async (reason) => {
      logger.log(`Device at id ${deviceInfo.id} is unfrozen. Reason: ${reason}`)
  ...
  async detectChanges({ initial }: { initial?: boolean } = {}) {
    // If detection is already in progress, wait for it and then run again
    if (this.detectChangesPromise) {
      await this.detectChangesPromise
      // Don't run again if not initial - the previous run already handled it
      if (!initial) {
        return
      }
    }

    this.detectChangesPromise = this.performDetectChanges({ initial })

    try {
      await this.detectChangesPromise
    } finally {
      this.detectChangesPromise = undefined
    }
  }

  private async performDetectChanges({ initial }: { initial?: boolean } = {}) {
  ...
  async request(
    ...
    if (!device.instance.isOpen) {
      logger.warn(
        `Device instance at id ${id} is not open. Attempting to open port before request.`
      )
      await device.instance.openAsync()
    } else if (device.instance.isOpening) {
      logger.info(
        `Device instance at id ${id} is currently opening. Waiting for it to open before request.`
      )
  ...
  private async waitForUnpause(id: SerialPortDeviceId): Promise<void> {
    ...
    while (device && device.requestsPaused) {
      ...
      logger.log(`Waiting for device at id ${id} to unpause...`)
    }
  }
