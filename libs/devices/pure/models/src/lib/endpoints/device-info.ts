...
export const PureInfoResponseValidator = z.object({
  ...
  batteryState: z.preprocess(Number, z.enum(BatteryState)),
  ...
  version: z.string(),
  selectedSim: z.preprocess(Number, z.enum(SIM)),
  signalStrength: z.preprocess(Number, z.enum(SignalStrength)),
  networkOperatorName: z.string(),
  trayState: z.preprocess(Number, z.enum(Tray)),
  accessTechnology: z.preprocess(Number, z.enum(AccessTechnology)),
  networkStatus: z.preprocess(Number, z.enum(NetworkStatus)),
  serialNumber: z.string(),
  caseColour: z.enum(CaseColor),
