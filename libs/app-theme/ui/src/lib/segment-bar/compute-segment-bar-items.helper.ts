...
interface AdjustedSegment extends ISegmentBarItem {
  usesMinWidth: boolean
}

export interface ComputedSegmentBarItem extends ISegmentBarItem {
  width: string
  left: string
  zIndex: number
}

const calculateTotalSegmentValue = (segments: ISegmentBarItem[]): number =>
  segments.reduce((sum, segment) => sum + segment.value, 0)

const calculateOverlapAdjustment = (segments: ISegmentBarItem[]): number =>
  segments.reduce(
