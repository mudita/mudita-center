  ...
  const checkForUpdateAndGetNewData = async () => {
    ...
    try {
      const { data } = await axios.get(
        `https://${process.env.MUDITA_WEB_CONTENTFUL_HOST}/spaces/${process.env.MUDITA_WEB_CONTENTFUL_SPACE_ID}/environments/${process.env.MUDITA_WEB_CONTENTFUL_ENVIRONMENT}/entries/?access_token=${process.env.MUDITA_WEB_CONTENTFUL_ACCESS_TOKEN}&content_type=newsItem&limit=3`
      )
