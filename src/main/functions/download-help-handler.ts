...
export const registerDownloadHelpHandler = () => {
  ...
  ipcMain.answerRenderer(HelpActions.DownloadContentfulData, async () => {
    const client = createClient({
      accessToken: process.env.MDA_CONTENTFUL_ACCESS_TOKEN as string,
      space: process.env.MDA_CONTENTFUL_SPACE_ID as string,
      environment: process.env.MDA_CONTENTFUL_ENVIRONMENT_ID,
      host: process.env.MDA_CONTENTFUL_HOST,
    })
