export const mockedTopics = [
  {
    id: "7352c250-ebe0-43f2-be31-0587e38e3bed",
    caller: {
      id: "655eeb91-ed31-4215-9842-e5eed886765e",
      firstName: "",
      lastName: "",
      primaryPhoneNumber: "+21 538 135 131",
      secondaryPhoneNumber: "+26 382 179 659",
      email: "Cameron.Dibbert57@gmail.com",
      note: "fuga minima error asperiore",
      ice: false,
      favourite: false,
      blocked: true,
      firstAddressLine: "9633 Jaylen Mountain",
      secondAddressLine: "",
    },
    unread: true,
    messages: [
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "e9eb31ea-b7e7-499e-a781-88e5f4dc6b0a",
        date: "2020-05-26T20:32:01.433Z",
        content:
          "Aut maiores dolore temporibus ex enim voluptas. Delectus et nemo fuga voluptas dolorum rem nostrum.",
        interlocutor: false,
      },
      {
        author: {
          id: "655eeb91-ed31-4215-9842-e5eed886765e",
          firstName: "",
          lastName: "",
          primaryPhoneNumber: "+21 538 135 131",
          secondaryPhoneNumber: "+26 382 179 659",
          email: "Cameron.Dibbert57@gmail.com",
          note: "fuga minima error asperiores",
          ice: false,
          favourite: false,
          blocked: true,
          firstAddressLine: "9633 Jaylen Mountain",
          secondAddressLine: "",
        },
        id: "78a5185c-4c72-4386-a24e-e1a5c0f9115b",
        date: "2020-01-26T12:51:57.798Z",
        content:
          "Libero ea ut quod. Est doloribus cupiditate nulla illum cumque in.",
        interlocutor: true,
      },
      {
        author: {
          id: "655eeb91-ed31-4215-9842-e5eed886765e",
          firstName: "",
          lastName: "",
          primaryPhoneNumber: "+21 538 135 131",
          secondaryPhoneNumber: "+26 382 179 659",
          email: "Cameron.Dibbert57@gmail.com",
          note: "fuga minima error asperiores",
          ice: false,
          favourite: false,
          blocked: true,
          firstAddressLine: "9633 Jaylen Mountain",
          secondAddressLine: "",
        },
        id: "6354c436-908e-475e-b2b5-eb729b314f2d",
        date: "2019-10-03T12:23:37.976Z",
        content:
          "Quas aut nihil facere minima totam animi distinctio dolores. Quia alias ut voluptas laborum doloribus enim harum ut.",
        interlocutor: true,
      },
      {
        author: {
          id: "655eeb91-ed31-4215-9842-e5eed886765e",
          firstName: "",
          lastName: "",
          primaryPhoneNumber: "+21 538 135 131",
          secondaryPhoneNumber: "+26 382 179 659",
          email: "Cameron.Dibbert57@gmail.com",
          note: "fuga minima error asperiores",
          ice: false,
          favourite: false,
          blocked: true,
          firstAddressLine: "9633 Jaylen Mountain",
          secondAddressLine: "",
        },
        id: "f80d59f3-d2da-4a2f-9823-fc4401476b89",
        date: "2019-10-24T16:58:46.590Z",
        content:
          "Similique voluptatem possimus numquam quidem. Maiores quo sed itaque.",
        interlocutor: true,
      },
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "45446b6f-bfdc-4f84-9513-d9e12fb701f9",
        date: "2019-09-03T09:01:08.080Z",
        content: "Est sit aut totam. Laboriosam ex nam sunt ipsa.",
        interlocutor: false,
      },
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "61a884a9-85a6-472c-811f-8dbab840c6f6",
        date: "2020-06-08T22:36:33.476Z",
        content:
          "Tenetur voluptatibus ab aperiam quia voluptatem et optio quae. Quia et reprehenderit aut quos itaque tempore.",
        interlocutor: false,
      },
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "a50f5489-ee89-4bcf-bf59-5dd012646de2",
        date: "2019-10-12T18:15:52.033Z",
        content:
          "Omnis totam ullam ea libero qui error excepturi. Facilis vel in totam tempore autem nihil eos.",
        interlocutor: false,
      },
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "40a35d29-f4d0-4ccb-a9d3-6cd7329a05a0",
        date: "2019-12-24T04:13:47.987Z",
        content:
          "Reiciendis ratione rerum suscipit. Ea quam enim atque et eveniet impedit explicabo reprehenderit sed.",
        interlocutor: false,
      },
      {
        author: {
          id: "655eeb91-ed31-4215-9842-e5eed886765e",
          firstName: "",
          lastName: "",
          primaryPhoneNumber: "+21 538 135 131",
          secondaryPhoneNumber: "+26 382 179 659",
          email: "Cameron.Dibbert57@gmail.com",
          note: "fuga minima error asperiores",
          ice: false,
          favourite: false,
          blocked: true,
          firstAddressLine: "9633 Jaylen Mountain",
          secondAddressLine: "",
        },
        id: "97d3fdfc-e4c0-42d5-9f30-c341bbf1dcbb",
        date: "2020-05-08T12:22:28.824Z",
        content:
          "Consequatur quasi ducimus dolorum animi facilis assumenda eaque sapiente beatae. Maiores adipisci molestiae.",
        interlocutor: true,
      },
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "50e01d46-0f90-41c8-a52a-aee2497f724a",
        date: "2020-03-08T11:46:37.118Z",
        content:
          "Dolores iure commodi et alias molestiae rerum excepturi odit tempora. Rem suscipit sint cum soluta porro.",
        interlocutor: false,
      },
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "c9b4431c-4eb1-45fd-9479-511ef5859dcb",
        date: "2020-06-16T07:49:04.669Z",
        content:
          "Voluptatum sed sed sequi omnis sint. Libero quia tempora est.",
        interlocutor: false,
      },
      {
        author: {
          id: "655eeb91-ed31-4215-9842-e5eed886765e",
          firstName: "",
          lastName: "",
          primaryPhoneNumber: "+21 538 135 131",
          secondaryPhoneNumber: "+26 382 179 659",
          email: "Cameron.Dibbert57@gmail.com",
          note: "fuga minima error asperiores",
          ice: false,
          favourite: false,
          blocked: true,
          firstAddressLine: "9633 Jaylen Mountain",
          secondAddressLine: "",
        },
        id: "de162537-7daf-43b2-b206-1c76daa2774d",
        date: "2019-08-07T23:50:25.111Z",
        content:
          "Quae sunt id et dolores eaque quidem. Culpa rem consequuntur alias reprehenderit.",
        interlocutor: true,
      },
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "16885b4c-eaf7-446f-9fb6-1125dd29ab42",
        date: "2020-01-27T02:49:15.506Z",
        content:
          "Ullam voluptatem enim aut consectetur ea facere beatae architecto. Et et est est non enim consequuntur.",
        interlocutor: false,
      },
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "1031f88f-4675-40c4-baf4-303fe1ee0fe0",
        date: "2020-06-01T13:53:27.087Z",
        content:
          "Exercitationem vel quasi doloremque. Enim qui quis quidem eveniet est corrupti itaque recusandae.",
        interlocutor: false,
      },
    ],
  },
  {
    id: "5a016ee9-1869-40ad-b2b7-a4c24cf2db7e",
    caller: {
      id: "162bf8b4-1190-4baf-9a9f-79b506cb30bc",
      firstName: "",
      lastName: "Lubowitz",
      primaryPhoneNumber: "+28 717 097 371",
      secondaryPhoneNumber: "+61 546 751 441",
      email: "Rebecca.Lubowitz@yahoo.com",
      note: "quo",
      ice: false,
      favourite: false,
      blocked: false,
      firstAddressLine: "",
      secondAddressLine: "Cormierville",
    },
    unread: true,
    messages: [
      {
        author: {
          id: "162bf8b4-1190-4baf-9a9f-79b506cb30bc",
          firstName: "",
          lastName: "Lubowitz",
          primaryPhoneNumber: "+28 717 097 371",
          secondaryPhoneNumber: "+61 546 751 441",
          email: "Rebecca.Lubowitz@yahoo.com",
          note: "quo",
          ice: false,
          favourite: false,
          blocked: false,
          firstAddressLine: "",
          secondAddressLine: "Cormierville",
        },
        id: "c0bb1f72-095f-4c17-a0c2-9a930a4a1595",
        date: "2019-11-11T03:11:20.794Z",
        content:
          "Quibusdam consequatur sit excepturi. Possimus et itaque perspiciatis amet autem eum quia sit.",
        interlocutor: true,
      },
      {
        author: {
          id: "162bf8b4-1190-4baf-9a9f-79b506cb30bc",
          firstName: "",
          lastName: "Lubowitz",
          primaryPhoneNumber: "+28 717 097 371",
          secondaryPhoneNumber: "+61 546 751 441",
          email: "Rebecca.Lubowitz@yahoo.com",
          note: "quo",
          ice: false,
          favourite: false,
          blocked: false,
          firstAddressLine: "",
          secondAddressLine: "Cormierville",
        },
        id: "378a2645-39e1-4ae9-85ef-71be3e23972a",
        date: "2020-03-12T20:27:42.487Z",
        content:
          "A numquam necessitatibus nam et. Provident dolorem nisi aut sint magnam quo voluptates aliquid.",
        interlocutor: true,
      },
      {
        author: {
          id: "162bf8b4-1190-4baf-9a9f-79b506cb30bc",
          firstName: "",
          lastName: "Lubowitz",
          primaryPhoneNumber: "+28 717 097 371",
          secondaryPhoneNumber: "+61 546 751 441",
          email: "Rebecca.Lubowitz@yahoo.com",
          note: "quo",
          ice: false,
          favourite: false,
          blocked: false,
          firstAddressLine: "",
          secondAddressLine: "Cormierville",
        },
        id: "c4cb85c3-a687-49bc-8891-9729ef754500",
        date: "2020-02-24T08:35:35.817Z",
        content:
          "Quia sit et ut quia quod nihil suscipit ducimus ut. Ut velit repudiandae qui maxime aperiam laboriosam et.",
        interlocutor: true,
      },
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "1f9fb1e8-bed1-48a5-a2c8-17dc0dc36f07",
        date: "2020-05-26T14:26:36.963Z",
        content:
          "Fugiat facere harum. Voluptatibus sit dolorem consequatur voluptas porro quo mollitia.",
        interlocutor: false,
      },
      {
        author: {
          id: "162bf8b4-1190-4baf-9a9f-79b506cb30bc",
          firstName: "",
          lastName: "Lubowitz",
          primaryPhoneNumber: "+28 717 097 371",
          secondaryPhoneNumber: "+61 546 751 441",
          email: "Rebecca.Lubowitz@yahoo.com",
          note: "quo",
          ice: false,
          favourite: false,
          blocked: false,
          firstAddressLine: "",
          secondAddressLine: "Cormierville",
        },
        id: "72cce1d4-bcfb-4664-96ba-fc47d5a981f0",
        date: "2019-11-24T07:47:27.654Z",
        content: "Et illum hic. Debitis enim sint in tempore nihil id.",
        interlocutor: true,
      },
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "749bf5d3-4ec1-40cb-859f-f22c3ccdf5e8",
        date: "2019-10-27T20:42:11.157Z",
        content:
          "Quos et deserunt placeat unde eum quaerat repudiandae veritatis id. Voluptatem mollitia voluptatem porro et qui illo aliquid.",
        interlocutor: false,
      },
      {
        author: {
          id: "162bf8b4-1190-4baf-9a9f-79b506cb30bc",
          firstName: "",
          lastName: "Lubowitz",
          primaryPhoneNumber: "+28 717 097 371",
          secondaryPhoneNumber: "+61 546 751 441",
          email: "Rebecca.Lubowitz@yahoo.com",
          note: "quo",
          ice: false,
          favourite: false,
          blocked: false,
          firstAddressLine: "",
          secondAddressLine: "Cormierville",
        },
        id: "6447b909-0efe-4b47-9995-4150cf3b5207",
        date: "2020-01-17T01:31:04.819Z",
        content:
          "Molestiae esse commodi rerum architecto. Accusamus quo minima deleniti earum eos totam non et id.",
        interlocutor: true,
      },
      {
        author: {
          id: "162bf8b4-1190-4baf-9a9f-79b506cb30bc",
          firstName: "",
          lastName: "Lubowitz",
          primaryPhoneNumber: "+28 717 097 371",
          secondaryPhoneNumber: "+61 546 751 441",
          email: "Rebecca.Lubowitz@yahoo.com",
          note: "quo",
          ice: false,
          favourite: false,
          blocked: false,
          firstAddressLine: "",
          secondAddressLine: "Cormierville",
        },
        id: "0a28e189-51d4-4c17-a6c0-9328fdba6007",
        date: "2020-01-07T19:51:50.835Z",
        content:
          "Voluptatibus nulla maiores ut quam qui tempore est et. Sequi cum fuga architecto repudiandae.",
        interlocutor: true,
      },
      {
        author: {
          id: "162bf8b4-1190-4baf-9a9f-79b506cb30bc",
          firstName: "",
          lastName: "Lubowitz",
          primaryPhoneNumber: "+28 717 097 371",
          secondaryPhoneNumber: "+61 546 751 441",
          email: "Rebecca.Lubowitz@yahoo.com",
          note: "quo",
          ice: false,
          favourite: false,
          blocked: false,
          firstAddressLine: "",
          secondAddressLine: "Cormierville",
        },
        id: "e48968b7-8818-4243-917a-cc3aa8516e49",
        date: "2020-05-30T14:19:03.875Z",
        content:
          "Et velit debitis nostrum commodi laborum mollitia. Eum dolorum neque blanditiis delectus modi et.",
        interlocutor: true,
      },
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "c9b42c87-3c2f-4fec-8d91-5c1507cbede7",
        date: "2020-03-04T13:39:00.756Z",
        content:
          "Voluptatem aut autem consectetur. Ut pariatur et quia tenetur qui.",
        interlocutor: false,
      },
      {
        author: {
          id: "162bf8b4-1190-4baf-9a9f-79b506cb30bc",
          firstName: "",
          lastName: "Lubowitz",
          primaryPhoneNumber: "+28 717 097 371",
          secondaryPhoneNumber: "+61 546 751 441",
          email: "Rebecca.Lubowitz@yahoo.com",
          note: "quo",
          ice: false,
          favourite: false,
          blocked: false,
          firstAddressLine: "",
          secondAddressLine: "Cormierville",
        },
        id: "7270bf34-f71a-4360-8a0d-492ee6f411a9",
        date: "2020-03-19T06:10:53.782Z",
        content: "Ex commodi quia iure. Maiores tempore error dolorum.",
        interlocutor: true,
      },
      {
        author: {
          id: "123",
          firstName: "John",
          lastName: "Doe",
          primaryPhoneNumber: "123 123 123",
        },
        id: "35c0783c-0424-4c8e-b608-f3f7546ea5bf",
        date: "2020-05-17T19:58:05.136Z",
        content:
          "Velit et ut ut odit quo. Ratione eligendi non consequatur ipsum neque.",
        interlocutor: false,
      },
    ],
  },
]
