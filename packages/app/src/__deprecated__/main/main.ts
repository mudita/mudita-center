...
ipcMain.answerRenderer(HelpActions.OpenWindow, () => {
  const title = "CP-101-test-v3 - Help"
  if (helpWindow === null) {
...
createOpenWindowListener(
  ...
  "CP-101-test-v3 - Terms of service",
  termsWindow
)
...
createOpenWindowListener(
  ...
  Mode.Sar,
  URL_OVERVIEW.sar,
  "CP-101-test-v3 - SAR information",
  policyWindow
)
...
ipcMain.answerRenderer(GoogleAuthActions.OpenWindow, async (scope: Scope) => {
  const title = "CP-101-test-v3 - Google Auth"
...
ipcMain.answerRenderer(
  ...
  async (data: { authorizationUrl: string; scope: string }) => {
    const title = "CP-101-test-v3 - Outlook Auth"
