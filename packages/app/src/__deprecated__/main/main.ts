...
ipcMain.answerRenderer(HelpActions.OpenWindow, () => {
  const title = "CP-101-test-v2 - Help"
  if (helpWindow === null) {
...
createOpenWindowListener(
  ...
  "CP-101-test-v2 - Terms of service",
  termsWindow
)
...
createOpenWindowListener(
  ...
  Mode.Sar,
  URL_OVERVIEW.sar,
  "CP-101-test-v2 - SAR information",
  policyWindow
)
...
ipcMain.answerRenderer(GoogleAuthActions.OpenWindow, async (scope: Scope) => {
  const title = "CP-101-test-v2 - Google Auth"
...
ipcMain.answerRenderer(
  ...
  async (data: { authorizationUrl: string; scope: string }) => {
    const title = "CP-101-test-v2 - Outlook Auth"
