...
describe("`trackOsVersion`", () => {
  describe("when `deviceType` is a `MuditaHarmony` and `serialNumber` exist", () => {
    const trackOsVersionOptions: TrackOsVersionOptions = {
      deviceType: DeviceType.MuditaHarmony,
      osVersion: "1.0.0",
      serialNumber: "1UB13213MN14K1",
    }

    test("`trackRequest` is called properly", async () => {
      await trackOsVersion(trackOsVersionOptions)
      expect(trackRequest).toHaveBeenCalledWith({
        _id: trackOsVersionOptions.serialNumber,
        e_c: TrackEventCategory.HarmonyVersion,
        e_a: trackOsVersionOptions.osVersion,
        [TrackEventDimension.HarmonyVersion]: trackOsVersionOptions.osVersion,
      })
    })
  })
  ...
  describe("when `deviceType` is a `MuditaPure` and `serialNumber` exist", () => {
    ...
    test("`trackRequest` is called properly", async () => {
      ...
      expect(trackRequest).toHaveBeenCalledWith({
        ...
        e_a: trackOsVersionOptions.osVersion,
        [TrackEventDimension.PureVersion]: trackOsVersionOptions.osVersion,
      })
    })
  })
