...
export class UsbParser {
  static buildContainerPacket(option: WriteOption): ArrayBuffer {
    ...
    const length =
      payloadOffset + payload.length * Uint32Array.BYTES_PER_ELEMENT

    const buffer = new ArrayBuffer(length)
    const bytes = new DataView(buffer)
    bytes.setUint32(0, length, true)
    bytes.setUint16(4, option.type, true)
    bytes.setUint16(6, option.code, true)
    bytes.setUint32(8, option.id, true)

    payload.forEach((element, index) => {
      const offset = payloadOffset + index * Uint32Array.BYTES_PER_ELEMENT
      bytes.setUint32(offset, element, true)
    })
    return buffer
  }
